...
const MockWLEDClient = WLEDClient as jest.MockedClass<typeof WLEDClient>;

describe('useClient Hook', () => {
  const mockOptions: ClientOptions = { baseUrl: 'mock-host' };
  let mockClientInstance: jest.Mocked<WLEDClient>;
  ...
  beforeEach(() => {
    ...
    } as unknown as jest.Mocked<WLEDClient>; // Type assertion needed for complex mock
  ...
  it('should update info when infoChange event is emitted', () => {
    let infoChangeCallback: (() => void) | null = null;
    const mockInfoData: WLEDMockInfo = { name: 'Test WLED', ver: '0.14.0' };
    ...
    act(() => {
      if (infoChangeCallback) infoChangeCallback();
    });
   ...
   it('should update state when stateChange event is emitted', () => {
    ...
    const mockStateData: WLEDMockState = { on: true, bri: 100 };
   ...
   it('should update effects when effectsChange event is emitted', () => {
    let effectsChangeCallback: (() => void) | null = null;
    ...
    act(() => {
      if (effectsChangeCallback) effectsChangeCallback();
    });
   ...
   it('should update palettes when palettesChange event is emitted', () => {
    let palettesChangeCallback: (() => void) | null = null;
    ...
    act(() => {
      if (palettesChangeCallback) palettesChangeCallback();
    });
